...
const sampleTemplate = `This template demonstrates printing a {{ .Variable | printf "%#v" }}.
{{if .Condition}}
...
strings.Split then immediately use the array created as a result:
...
{{/*
This is a way to insert a 
multi-line comment
*/}}`

const secondTemplate = `{{define "block_example"}}
{{.OtherVariable}}
{{end}}`
...
func RunTemplate() error {
	...
	// to demonstrate blocks we will create another template
	// by cloning the first template, then passing a second
	t2, err := t.Clone()
	if err != nil {
		return err
	}
