...
// InitTemplate sets up templates from a directory
func InitTemplates() error {
	...
	err = CreateTemplate(filepath.Join(tempdir, "t1.tmpl"), `Template 1! {{ .Var1 }}
{{ block "template2" .}} {{end}}
{{ block "template3" .}} {{end}}`)
	if err != nil {
		return err
	...
	// parse glob will combine all files that match
