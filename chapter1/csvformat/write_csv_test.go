...
func TestBooks_ToCSV(t *testing.T) {
	tests := []struct{
		...
		{
			name: "base-case",
			books: &Books{{"F Scott Fitzgerald", "The Great Gatsby"},},
...
func TestWriteCSVOutput(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			if err := WriteCSVOutput(); (err != nil) != testCase.wantErr {
				t.Errorf("WriteCSVOutput() error = %v, wantErr %v", err, testCase.wantErr)
			}
		})
	}
}
...
func TestWriteCSVBuffer(t *testing.T) {
	tests := []struct{
		...
		{"base-case",
		bytes.NewBufferString("Author,Title\nF Scott Fitzgerald,The Great Gatsby\nJ D Salinger,The Catcher in the Rye\n"),
		false},
	}
